...
int slqueue_front(SLQueue queue){
  return slqueue_data(queue);
}

SLQueue slqueue_enqueue(SLQueue queue, int data){
  SLQueue newQueue = malloc(sizeof(SLQueueNode));
  slqueue_data(newQueue) = data;
  slqueue_next(newQueue) = slqueue_nil();

  if(!queue) return newQueue;
