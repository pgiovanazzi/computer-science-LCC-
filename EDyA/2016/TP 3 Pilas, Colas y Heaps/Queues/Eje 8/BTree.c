...
void btree_foreach_level(BTree *tree, VisitorFuncInt visit, void *extra_data){
	...
	while (queue) {
		...
		/* una forma es esta:
		BTree *nodeTmp = (BTree *)slqueue_front(queue);
		queue = slqueue_dequeue(queue);
		if (btree_left(nodeTmp))
			queue = slqueue_enqueue(queue,btree_left(nodeTmp));
		if(btree_right(nodeTmp))
			queue = slqueue_enqueue(queue,btree_right(nodeTmp));
		*/
		// otra forma es esta:
		if( btree_left((BTree *)slqueue_front(queue)) )
			queue = slqueue_enqueue(queue, btree_left((BTree *)slqueue_front(queue)));
		if( btree_right((BTree *)slqueue_front(queue)) )
